...
// Legacy handlers implementation (self-contained in this file).
...
axios_1.default.defaults.timeout = axios_1.default.defaults.timeout || 120000;
// Helper: determine whether a model name looks like a cloud model
function looksLikeCloudModelName(n) {
    if (!n)
        return false;
...
function modelObjIsSafe(m) {
    if (!m)
        return false;
    const name = m.name || '';
    if (looksLikeCloudModelName(name))
        return true;
    // Only accept models that are actually installed locally (not just available for download)
    if (m.installed || m.local || m.downloaded)
        return true;
    return false;
}
...
async function isModelAvailable(modelName) {
    ...
    if (!STRICT)
        return true;
    if (looksLikeCloudModelName(modelName))
        return true;
    try {
        const resp = await axios_1.default.get(`${OLLAMA_BASE_URL}/api/tags`);
        const models = resp.data.models || [];
        const found = models.find((m) => m && m.name === modelName);
        if (!found)
            return false;
    ...
    switch (name) {
        ...
        case 'list_ollama_models': {
            try {
                ...
                const safe = rawModels.filter((m) => modelObjIsSafe(m)).map((m) => m.name);
                let modelsList = [];
                if (safe.length > 0) {
                    modelsList = safe;
                }
                    ...
                    const cloudOnly = rawModels
                        .filter((m) => m && looksLikeCloudModelName(m.name))
                        .map((m) => m.name);
                    modelsList = cloudOnly.length > 0 ? cloudOnly : rawModels.map((m) => m.name);
                }
                const note = safe.length > 0
            ...
            if (Array.isArray(modelsArg) && modelsArg.length > 0) {
                ...
                try {
                    ...
                    modelsToUse =
                        candidates.length > 0
                            ? candidates
                            : (resp.data.models || []).map((m) => m.name).slice(0, 2);
                }
                catch (_err) {
                    void _err;
                    modelsToUse = ['llama2'];
        ...
        case 'remember_consult': {
            ...
            try {
                const vscodeMcpPath = process.env.VSCODE_MCP_JSON ||
                    path_1.default.join(os_1.default.homedir(), 'Library', 'Application Support', 'Code', 'User', 'mcp.json');
                const raw = await promises_1.default.readFile(vscodeMcpPath, 'utf-8').catch(() => '');
                if (raw) {
                    try {
                        ...
                        if (rememberKey)
                            tryServers.push(servers[rememberKey]);
                        if (memoryKey)
                            tryServers.push(servers[memoryKey]);
                    }
                    catch (_e) {
                        void _e;
                        // ignore parse errors
            ...
            async function tryCallMemoryServer(serverCfg) {
                ...
                for (const p of patterns) {
                    const found = tools.find((t) => t.name && t.name.toLowerCase().includes(p));
                    if (found) {
                        chosen = found;
                        break;
                    }
                ...
                if (!chosen && tools.length > 0) {
                    chosen =
                        tools.find((t) => t.inputSchema && t.inputSchema.type === 'object') || tools[0];
                }
                if (!chosen) {
                    await client.close();
                ...
                if (props['response'] !== undefined ||
                    ...
                    if (props['text'] !== undefined)
                        toolArgs.text = payload.response;
                    if (props['value'] !== undefined)
                        toolArgs.value = payload.response;
                    if (props['content'] !== undefined)
                        toolArgs.content = payload.response;
                }
                ...
                const callRes = await client
                    .callTool({ name: chosen.name, arguments: toolArgs }, types_js_1.CallToolResultSchema)
            ...
            try {
                ...
                await promises_1.default.writeFile(filePath, JSON.stringify(payloadToWrite, null, 2), 'utf-8');
                return {
                    content: [
                        {
                            type: 'text',
