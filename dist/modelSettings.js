...
exports.suggestConsultSettings = suggestConsultSettings;
exports.shouldAutoModelSettings = shouldAutoModelSettings;
function clamp(n, min, max) {
    return Math.max(min, Math.min(max, n));
}
function looksLikeCloudModel(name) {
...
function parseModelName(modelName) {
    ...
    if (sizeMatch) {
        ...
        if (unit === 'k' || unit === 'kb') {
            ...
            info.size = `${parseFloat(num) / 1000000}B`;
        }
        else if (unit === 'g' || unit === 'gb') {
            info.size = `${parseFloat(num) / 1000000000}B`;
        }
        else if (unit === 't' || unit === 'tb') {
            info.size = `${parseFloat(num) / 1000000000000}B`;
        }
        else {
...
function sizeToNumberBillions(size) {
 ...
 */
function suggestConsultSettings(args) {
    ...
    timeoutMs = clamp(timeoutMs, 1000, 600000);
...
function shouldAutoModelSettings(params) {
