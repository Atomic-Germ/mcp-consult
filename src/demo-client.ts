...
async function main() {
  ...
  try {
    ...
    try {
      const content = listRes.content || [];
      const firstText = (Array.isArray(content) ? content : []).find((c: any) => c.type === "text")?.text || "";
    ...
    try {
      consultRes = await withTimeout(
        client.callTool({ name: "consult_ollama", arguments: { model: modelName, prompt } }, CallToolResultSchema),
        15_000
      );
    } catch (err: any) {
    ...
    if (consultRes) {
      ...
      try {
        ...
        const modelsMatch = firstText.match(/Available models:\s*(.*)/i);
        if (modelsMatch && modelsMatch[1]) {
          availableModels = modelsMatch[1].split(/[,;\n]+/).map((s: string) => s.trim()).filter(Boolean);
        }
      } catch (e) {
        // ignore
      }
      ...
      try {
        const compareRes = await client.callTool({ name: "compare_ollama_models", arguments: compareArgs }, CallToolResultSchema);
